...
class LlamaLocalInference(
    ...
    fun inferenceStartWithAgent(
        agentId: String,
        sessionId: String,
        turnService: TurnService,
        prompt: ArrayList<Message>,
        ctx: Context
    ): String {
    ...
    fun inferenceStartWithoutAgent(
        modelName: String,
    ...
    private fun inferenceCall(
        ...
        AppLogging.getInstance()
            .log("local inference with prompt=${conversationHistory.last().text}")
        val sequenceLength =
            ModelUtils.getSequenceLengthForConversationHistory(conversationHistory, systemPrompt)
        var response = ""
        if (streaming) {
            val result = client!!.inference().chatCompletionStreaming(
                InferenceChatCompletionParams.builder()
                    ...
                    .messages(
                        constructLSMessagesFromConversationHistoryAndSystemPrompt(
                            conversationHistory,
                            systemPrompt
                        )
                    )
    ...
    fun createLocalAgent(
        ...
        tokenizerPath: String,
        temperature: Double,
        userProvidedSystemPrompt: String,
        ctx: Context
    ): Triple<String, String, TurnService> {
        AppLogging.getInstance().log("Creating local agent")
        val agentConfig = createLocalAgentConfig(
            modelName,
            modelPath,
            tokenizerPath,
            temperature,
            userProvidedSystemPrompt
        )
    ...
    private fun localAgentInference(
        agentId: String,
        sessionId: String,
        turnService: TurnService,
        conversationHistory: ArrayList<Message>,
        ...
        val messagesAndDocuments: Pair<List<AgentTurnCreateParams.Message>, List<String>> =
            constructMessagesDocumentsForAgent(conversationHistory)
            ...
            agentTurnCreateResponseStream.asSequence().forEach {
                ...
                when {
                    ...
                    agentResponsePayload.isAgentTurnResponseStepProgress() -> {
                        ...
                        val result =
                            agentResponsePayload.agentTurnResponseStepProgress()?.delta()?.text()
                                ?.text()
                        if (result != null) {
                    ...
                    agentResponsePayload.isAgentTurnResponseStepComplete() -> {
                        ...
                        val toolCalls =
                            agentResponsePayload.agentTurnResponseStepComplete()?.stepDetails()
                                ?.asInferenceStep()?.modelResponse()?.toolCalls()
    ...
    private fun createLocalAgentConfig(
        modelName: String,
        modelPath: String,
        tokenizerPath: String,
        temperature: Double,
        ...
        val zdt = ZonedDateTime.ofInstant(
            Instant.parse(Clock.System.now().toString()),
